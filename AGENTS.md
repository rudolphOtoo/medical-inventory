# Antigravity Agent Configuration & Customization Index

This workspace is integrated with the **Conscience OS** methodology and skill suite located in [`.agents/skills/`](file:///c:/Users/doks/Herd/medtrack/.agents/skills).

## Available Slash Commands & Skills

| Command / Skill | Scope & Purpose |
|---|---|
| **`/architect`** | Plan before you build: asks clarifying questions one at a time and creates comprehensive implementation plans. |
...
| **`/review`** | Post-implementation QA check against plan and boundaries grouped by severity without auto-fixing. |
| **`/recover`** | Root cause diagnosis for terminal errors and broken states with single-shot fixes. |
| **`/imprint`** | UI consistency tracking: captures component patterns into the UI registry. |
| **`/impeccable`** | Design & UI excellence system for frontend craftsmanship, UX polish, and typography. |
| **`/pipeline`** | Applied Mathematician + PM + Senior Engineer problem-solving pipeline with Elm architecture & TDD. |
| **`/graphify-ide`** | Inline IDE semantic extraction & graph generation workflow without external API keys. |
| **`/remember`** | Session state management (`save` to compress context, `restore` to resume without re-explaining). |
| **`/session-handoff`** | End-of-session compact handoff filter (Layer A paste block + Layer B HANDOFF.md). |
| **`/ste-writing`** | ASD-STE100 Simplified Technical English converter for clear, anti-slop documentation. |
