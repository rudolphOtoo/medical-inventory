            ...
            <div class="flex flex-wrap items-center gap-2 font-mono text-xs">
                ...
                @if (auth()->user()->isAdmin())
                    <button
                        ...
                        class="inline-flex items-center gap-1.5 rounded-lg border border-slate-300 dark:border-[#2c303d] bg-white dark:bg-[#12141a] px-3 py-1.5 text-xs font-semibold text-slate-700 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-[#181a22] hover:text-slate-900 dark:hover:text-white transition cursor-pointer shadow-xs"
                    >
                        <x-ui.icon name="building" class="size-3.5" />
                ...
                @if (auth()->user()->isAdmin())
                    <!-- Archive Toggle -->
                    <form method="POST" action="{{ route('equipment.archive', $equipment) }}" onsubmit="return confirm('Toggle archive status for this equipment?');" class="inline">
                        @csrf
                        <button
                            type="submit"
                            class="inline-flex items-center gap-1.5 rounded-lg border border-slate-300 dark:border-[#2c303d] bg-white dark:bg-[#12141a] px-3 py-1.5 text-xs font-semibold text-slate-700 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-[#181a22] hover:text-slate-900 dark:hover:text-white transition cursor-pointer shadow-xs"
                        >
                            <span>{{ $equipment->is_archived ? __('Restore') : __('Archive') }}</span>
                        </button>
                    </form>
                    ...
                    <form method="POST" action="{{ route('equipment.destroy', $equipment) }}" onsubmit="return confirm('Permanently delete {{ $equipment->name }} [{{ $equipment->asset_tag }}]? All maintenance history and attachments will be deleted.');" class="inline">
                        ...
                        <button
                            type="submit"
                            class="inline-flex items-center gap-1.5 rounded-lg border border-rose-300 dark:border-rose-900/60 bg-rose-50 dark:bg-rose-950/40 px-3 py-1.5 text-xs font-semibold text-rose-700 dark:text-rose-300 hover:bg-rose-100 dark:hover:bg-rose-900/60 transition cursor-pointer shadow-xs"
                        >
                ...
                <div class="rounded-xl border border-slate-200 dark:border-[#1c1f26] bg-white dark:bg-[#0c0d10] p-6 space-y-4 shadow-xs">
                    ...
                    @if (auth()->user()->isAdmin() || auth()->user()->department_id === $equipment->department_id)
                        <button
                            ...
                            class="w-full inline-flex items-center justify-center gap-2 rounded-lg border border-slate-300 dark:border-[#2c303d] bg-white dark:bg-[#12141a] py-2 font-mono text-xs font-semibold text-slate-700 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-[#181a22] hover:text-slate-900 dark:hover:text-white transition cursor-pointer shadow-xs"
                        >
                            <x-ui.icon name="flask" class="size-3.5" />
