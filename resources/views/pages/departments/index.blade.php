                        ...
                        <div class="flex items-center gap-3">
                            ...
                            @if (auth()->user()->isAdmin() && ($dept->equipment_count ?? 0) === 0 && ($dept->staff_count ?? 0) === 0)
                                <form method="POST" action="{{ route('departments.destroy', $dept) }}" onsubmit="return confirm('Permanently delete department {{ $dept->name }}?');">
                                    @csrf
                                    @method('DELETE')
                                    <button type="submit" class="text-slate-500 hover:text-rose-400 transition cursor-pointer" title="Delete Ward">
                                        ✕
                                    </button>
                                </form>
                            @endif
                ...
                >
                    <div class="flex items-center justify-between border-b border-[#1c1f26] pb-3">
                        <div class="flex items-center gap-2">
                            ...
                            <h3 class="font-mono text-xs font-bold text-white uppercase tracking-wider">{{ __('Edit Ward Department') }}</h3>
                        </div>
                        <button
                            type="button"
                        ...
                        <div class="grid grid-cols-2 gap-3">
                            <div>
                                ...
                                <input
                                    ...
                                    required
                                    class="w-full rounded-lg border border-[#22262f] bg-[#08090a] px-3.5 py-2 text-xs text-white uppercase focus:border-slate-400 focus:outline-hidden font-mono"
                                />
                            </div>
                            <div>
                                <label class="block font-mono text-[10px] uppercase tracking-widest text-slate-400 mb-1">{{ __('Contact Ext.') }}</label>
