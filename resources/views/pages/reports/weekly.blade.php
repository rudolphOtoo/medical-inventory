<x-layouts.app :title="__('Weekly Operational Report')">
    <div class="space-y-6">
        ...
        <div class="flex flex-col sm:flex-row sm:items-end sm:justify-between gap-4 border-b border-[#1c1f26] pb-6">
            <div>
                <div class="flex items-center gap-2 font-mono text-[10px] uppercase tracking-widest text-slate-500 mb-1">
                    ...
                    <span>/</span>
                    <span class="text-slate-300">Weekly Hospital Operations Digest</span>
                </div>
                <h1 class="text-2xl font-bold tracking-tight text-white">{{ __('Weekly Executive Report') }}</h1>
                <p class="font-mono text-xs text-slate-400 mt-1">
                    Period: <span class="text-white font-semibold">{{ $report['startDate'] }}</span> to <span class="text-white font-semibold">{{ $report['endDate'] }}</span>
                </p>
            </div>
            ...
            <div class="flex flex-wrap items-center gap-2">
                <a
                    ...
                    target="_blank"
                    class="inline-flex items-center gap-2 rounded-lg border border-[#2c303d] bg-[#12141a] px-3.5 py-2 text-xs font-semibold text-slate-200 hover:bg-[#181a22] hover:text-white transition font-mono"
                >
                ...
                <a
                    ...
                    class="inline-flex items-center gap-2 rounded-lg bg-emerald-500 hover:bg-emerald-400 px-4 py-2 text-xs font-bold text-black transition cursor-pointer shadow-sm font-mono"
                >
        ...
        <div class="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 rounded-xl border border-[#1c1f26] bg-[#0c0d10] divide-y sm:divide-y-0 sm:divide-x divide-[#1c1f26]">
            ...
            <div class="p-5">
                ...
                <div class="mt-2 flex items-baseline gap-1">
                    <span class="font-mono text-3xl font-bold tracking-tight text-emerald-400">{{ $report['complianceRate'] }}%</span>
                </div>
            ...
            <div class="p-5">
                ...
                <div class="mt-2 flex items-baseline gap-1">
                    <span class="font-mono text-3xl font-bold tracking-tight text-white">{{ $report['totalEquipment'] }}</span>
            ...
            <div class="p-5">
                ...
                <div class="mt-2 flex items-baseline gap-1">
                    <span class="font-mono text-3xl font-bold tracking-tight {{ $report['underReviewCount'] > 0 ? 'text-amber-400' : 'text-slate-400' }}">
            ...
            <div class="p-5">
                ...
                <div class="mt-2 flex items-baseline gap-1">
                    <span class="font-mono text-3xl font-bold tracking-tight {{ $report['overdueCalibrations'] > 0 ? 'text-rose-400' : 'text-emerald-400' }}">
            ...
            <div class="p-5">
                ...
                <div class="mt-2 flex items-baseline gap-1">
                    <span class="font-mono text-3xl font-bold tracking-tight text-sky-400">{{ $report['weeklyTicketsCount'] }}</span>
                    <span class="font-mono text-[11px] text-slate-500">tickets</span>
                </div>
                <span class="font-mono text-[10px] text-emerald-400 mt-1 block">{{ $report['weeklyResolvedCount'] }} Resolved</span>
            </div>
            ...
            <div class="p-5">
                ...
                <div class="mt-2 flex items-baseline gap-1">
                    <span class="font-mono text-3xl font-bold tracking-tight text-white">{{ $report['avgMttrMinutes'] }}</span>
        ...
        <div class="grid gap-6 lg:grid-cols-2">
            ...
            <div class="rounded-xl border border-[#1c1f26] bg-[#0c0d10] p-6 space-y-4">
                <div class="flex items-center justify-between border-b border-[#1c1f26] pb-3">
                    <h3 class="font-mono text-xs font-bold text-white uppercase tracking-wider">{{ __('Ward Fleet Distribution & Uptime') }}</h3>
                </div>
                        ...
                        <tbody class="divide-y divide-[#1c1f26]">
                            @foreach ($report['departments'] as $dept)
                                ...
                                <tr class="hover:bg-[#12141a]/60">
                                    ...
                                    <td class="py-2.5 px-3 text-center font-mono text-emerald-400">{{ $active }}</td>
                                    <td class="py-2.5 px-3 text-center font-mono {{ $dept->issues_count > 0 ? 'text-amber-400' : 'text-slate-500' }}">{{ $dept->issues_count }}</td>
                                    <td class="py-2.5 px-3 text-right font-mono font-bold {{ $pct >= 90 ? 'text-emerald-400' : ($pct >= 75 ? 'text-amber-400' : 'text-rose-400') }}">
                                        {{ $pct }}%
                                    </td>
                                </tr>
                            @endforeach
            ...
            <div class="rounded-xl border border-[#1c1f26] bg-[#0c0d10] p-6 space-y-4">
                <div class="flex items-center justify-between border-b border-[#1c1f26] pb-3">
                    <h3 class="font-mono text-xs font-bold text-white uppercase tracking-wider">{{ __('Resolved Service Requests') }}</h3>
                </div>
                    ...
                    <div class="divide-y divide-[#1c1f26]">
                        @foreach ($report['recentResolvedTickets'] as $ticket)
                            <div class="py-2.5 flex items-center justify-between gap-3 first:pt-0 last:pb-0">
                                <div class="min-w-0">
                                    <span class="text-xs font-semibold text-white truncate block">{{ $ticket->title }}</span>
                                    <span class="font-mono text-[10px] text-slate-400">
                                        [{{ $ticket->equipment->asset_tag ?? 'N/A' }}] {{ $ticket->department->name ?? 'Ward' }} &middot; Resolved by {{ $ticket->assignee->name ?? 'Biomed Tech' }}
                                    </span>
                                </div>
                                <div class="font-mono text-[11px] text-emerald-400 shrink-0">
                                    ✓ Closed
                                </div>
                            </div>
                        @endforeach
